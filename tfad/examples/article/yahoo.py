...
def yahoo_pipeline(
    ...
    suspect_window_length: int = 5,
    validation_portion: float = 0.2,
    test_portion: float = 0.3,
    ...
    tcn_maxpool_out_channels: int = 16,
    embedding_rep_dim: int = 150,
    ...
    stride_roll_pred_val_test: int = 1,
    ...
    **kwargs,
    ...
    # Separate digits in benchmark_num, allowing to pass more than one with the parser
    if benchmark_num not in [1, 2, 3, 4]:
        benchmark_num = [int(d) for d in str(benchmark_num)]
    # Load benchmark dataset
    base_dataset = tfad.datasets.yahoo(
        path=data_dir,
    ...
    base_dataset = TimeSeriesDataset(
        tfad.utils.take_n_cycle(scaler(base_dataset), len(base_dataset))
    )
    # Number of channels in TimeSeries
    ts_channels = base_dataset[0].shape[1]
    assert all(shape[1] == ts_channels for shape in base_dataset.shape)

    # Split dataset
    train_set, validation_set, test_set = tfad.ts.split_train_val_test(
        data=base_dataset,
    ...
    # print(test_set[0])
    # print(type(test_set[0].values))
    # print(test_set[0].values)
    # print(test_set[0].labels)
    print(len(test_set))
    import pandas as pd
    for i in range(len(test_set)):
        temp = pd.DataFrame({'values': test_set[i].values, 'labels': test_set[i].labels})
        temp.to_csv('test_set'+str(i)+'.csv')

    #### inject anomalies on train dataset ###
    train_set_transformed = yahoo_inject_anomalies(
        dataset=train_set,
    ...
    print(f"TFAD on Yahoo dataset finished successfully!")
...
if __name__ == "__main__":
    ...
    # Yahoo specific parsing:
    parser.add_argument("--benchmark_num", type=int)
